...
exports.contactWebhook = async (req, res) => {
    ...
    try {
        ...
        const messageId = data?.message?.id || req.body.id;
        ...
        if (event === "contact.created" || event === "contact.updated") {
            ...
            if (items.length === 0) {
                console.log("Creating Monday contact...");
        ...
        if (event === "message.sent" || event === "message.received") {
            ...
            if (items.length === 0) {
                console.log("No Monday item found");
                return res.status(200).json({ success: true });
            }

            const itemId = items[0].id;

            const direction = event === "message.sent" ? "OUTGOING SMS" : "INCOMING SMS";

            const senderName = event === "message.sent"
                ? data.user?.full_name || "Unknown User"
                : data.contact?.full_name || "Unknown Contact";

            const senderPhone = event === "message.sent"
                ? data.number?.formatted_number || ""
                : data.contact?.formatted_number || "";

            const receiverName = event === "message.sent"
                ? data.contact?.full_name || "Unknown Contact"
                : data.user?.full_name || data.inbox?.name || "Unknown User";

            const receiverPhone = event === "message.sent"
                ? data.contact?.formatted_number || ""
                : data.number?.formatted_number || "";
...
`;
            ...
            console.log("Creating Monday update:", itemId);
            ...
            // ===============================
            let imageUrl = null;
            const payloadImage = findImageInPayload(data.message) || findImageInPayload(data);
            const shouldCheckForImage = messageMayHaveMedia(data.message, payloadImage);

            if (shouldCheckForImage) {

                console.log("Checking for image attachment...");
            ...
            const timelineBody =
                update.replace(/\n/g, "<br>") +
                (imageUrl ? `<br><a href="${imageUrl}">Image attachment</a>` : "");
